...
def update_q_table(cnt, current_state):
    ...
    if (cnt >= 30000 and cnt % 100 == 0) or (cnt < 30000):
...
for cnt in range(100000):
    ...
    ray_sensor_left = LineString([(x, y), (x + cos(q + (pi / 3)) * 2 * W, y + sin(
        q + (pi / 3)) * 2 * H)])  # a line from robot to a point outside arena in direction of q
